//
//  noteUserListTableViewCell.swift
//  tawkTo
//
...
class noteUserListTableViewCell: UITableViewCell {
    ...
    var noteIndicatorImageView = UIImageView()

    var model: notesElement!
    ...
    func initViews(){
        ...
        noteIndicatorImageView.image = #imageLiteral(resourceName: "notes")
        noteIndicatorImageView.translatesAutoresizingMaskIntoConstraints = false
        containerView.addSubview(noteIndicatorImageView)
        
        NSLayoutConstraint.activate([
            noteIndicatorImageView.trailingAnchor.constraint(equalTo: containerView.trailingAnchor, constant: -16),
            noteIndicatorImageView.centerYAnchor.constraint(equalTo: containerView.centerYAnchor),
            noteIndicatorImageView.widthAnchor.constraint(equalToConstant: 28.0),
            noteIndicatorImageView.heightAnchor.constraint(equalToConstant: 28.0)
        ])
        ...
        NSLayoutConstraint.activate([
            ...
            userNameAndDetailsStackView.trailingAnchor.constraint(equalTo: noteIndicatorImageView.trailingAnchor, constant: -16),
...
extension noteUserListTableViewCell:CustomElementCell{
    func configure(withModel: CustomElementModel, indexPath: IndexPath) {
        guard let model = withModel as? notesElement else {
