//
//  invertedUserListTableViewCell.swift
//  tawkTo
//
...
class invertedUserListTableViewCell: UITableViewCell {
...
extension invertedUserListTableViewCell:CustomElementCell{
    func configure(withModel: CustomElementModel, indexPath: IndexPath) {
        guard let model = withModel as? invertedElement else {
            ...
            if let url = URL(string: urlString){
                imageContainerView.imageView.loadImageWithUrl(url) { (image) in
                    DispatchQueue.main.async {
                        let invertImage = (image ?? #imageLiteral(resourceName: "userPlaceholder")).invertedImage()
                        self.imageContainerView.imageView.image = invertImage
                    }
                }
            }
        }
